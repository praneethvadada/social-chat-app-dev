...
public class FCMService {
    ...
    /**
     * TRIGGER: Incoming call (1:1 or group — pass groupId/groupName for group
     * calls, null/null for 1:1). data.type stays "CALL_INVITE" for both so
     * the existing native Android ringtone-trigger logic (which keys off
     * that field) keeps working unchanged; isGroupCall/groupId/groupName are
     * additive fields the Flutter side can branch on for copy/routing.
     * Call this in CallSignalingController.
     */
    public void onIncomingCall(Long recipientUserId, Long callerUserId, String channelId, boolean isVideo,
        ...
        try {
            ...
            if (isGroupCall) {
                ...
                if (groupName != null) data.put("groupName", groupName);
            }

            String title = isGroupCall ? (groupName != null ? groupName : "Group Call") : "Incoming Call";
            String callKind = isVideo ? "video" : "audio";
            String body = isGroupCall
                    ? callerName + " started a group " + callKind + " call"
                    : callerName + " is calling...";

            sendNotificationToUser(recipientUserId, title, body, data);

            log.info("[FCM] 📞 Call notification sent to user {} from caller {} (video={}, group={})",
                    recipientUserId, callerUserId, isVideo, groupId);
        } catch (Exception e) {
    ...
    /**
     * TRIGGER: A 1:1 call timed out unanswered (client marked it MISSED via
     * PUT /calls/{id}/status). Covers "receiver backgrounded but app alive";
     * does not cover both-devices-killed (would need a server scheduler).
     */
    public void onMissedCall(Long receiverUserId, Long callerUserId, boolean isVideo) {
        try {
            String callerName = getUserUsername(callerUserId);
            if (callerName == null) return;
            ...
            log.info("[FCM] 📵 Missed-call notification sent to user {} for caller {}", receiverUserId, callerUserId);
        } catch (Exception e) {
            log.error("[FCM] Error in onMissedCall: {}", e.getMessage());
        }
    }

    /**
     * TRIGGER: A group call ended while this user was still INVITED/RINGING
     * (never joined). Called from CallService.leaveParticipant when the last
     * joined participant leaves.
     */
    public void onMissedGroupCall(Long userId, Long groupId, String groupName) {
        try {
            String name = groupName != null ? groupName : "a group";

            Map<String, String> data = new HashMap<>();
            data.put("type", "MISSED_GROUP_CALL");
            data.put("groupId", groupId.toString());

            sendNotificationToUser(
                    userId,
                    "Missed Group Call",
                    "You missed a group call in " + name,
                    data
            );

            log.info("[FCM] 📵 Missed-group-call notification sent to user {} for group {}", userId, groupId);
        } catch (Exception e) {
            log.error("[FCM] Error in onMissedGroupCall: {}", e.getMessage());
        }
    }
