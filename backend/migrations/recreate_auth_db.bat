@echo off
REM Recreate auth_db from schema and apply migrations (Windows)
...
pauseendlocal
necho All migrations applied. You may now restart backend services.
necho To restart services, run your usual service scripts (e.g. start-all-services.bat).mysql -u %MYSQL_USER% -p%MYSQL_PASS% auth_db < "%CD%\backend\migrations\2025-12-28-add-message-clientid-readat.sql"
necho Applying code-required message migration (2025-12-28-add-message-clientid-readat.sql)...mysql -u %MYSQL_USER% -p%MYSQL_PASS% auth_db < "%CD%\backend\migrate_read_receipts.sql"
necho Applying read receipts migration (migrate_read_receipts.sql)...mysql -u %MYSQL_USER% -p%MYSQL_PASS% auth_db < "%CD%\backend\add_profile_columns.sql"
necho Applying profile columns migration (add_profile_columns.sql)...if errorlevel 1 (
  echo Failed importing schema.sql. Aborting.
  exit /b 1
)mysql -u %MYSQL_USER% -p%MYSQL_PASS% auth_db < "%CD%\backend\schema.sql"
necho Importing base schema (schema.sql)...if errorlevel 1 (
...
necho Dropping and recreating auth_db...set BACKUP_FILE="%CD%\auth_db_backup_%DATE:~10,4%%DATE:~4,2%%DATE:~7,2%.sql"
necho Backing up existing auth_db to %BACKUP_FILE%
nmysqldump -u %MYSQL_USER% -p%MYSQL_PASS% auth_db > %BACKUP_FILE%
@echo off
REM Recreate auth_db from schema and apply chat-only migrations (Windows)
REM Usage: run this from the workspace root in an elevated PowerShell/CMD prompt.
