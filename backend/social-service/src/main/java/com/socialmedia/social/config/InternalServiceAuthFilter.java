 ...
 * (/blocks/internal/**, /profiles/internal/**) that SecurityConfig
 * deliberately keeps out of the normal JWT/anyRequest().authenticated()
 * flow — they're called by sibling services (chats-service), not end-user
 * clients. Mirrors auth-service's InternalServiceAuthFilter exactly — see
 * its own doc comment for the full rationale (these paths were reachable
 * from the public internet through the gateway with zero gating).
 */
@Component
